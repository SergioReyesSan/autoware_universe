  ...
  updater_(this)
{
  ...
  publish_low_visibility_voxels_ =
    declare_parameter<bool>("publish_low_visibility_voxels", false);
...
std::vector<PolarVoxelIndex>
PolarVoxelOutlierFilterComponent::collect_low_visibility_candidates(
  ...
  const PolarVoxelIndex & voxel_idx) const
{
  ...
  covariance << sum_xx / point_count_d - mean_x * mean_x,
    sum_xy / point_count_d - mean_x * mean_y, sum_xz / point_count_d - mean_x * mean_z,
    sum_xy / point_count_d - mean_x * mean_y, sum_yy / point_count_d - mean_y * mean_y,
    sum_yz / point_count_d - mean_y * mean_z, sum_xz / point_count_d - mean_x * mean_z,
    sum_yz / point_count_d - mean_y * mean_z, sum_zz / point_count_d - mean_z * mean_z;
  ...
  const std::vector<rclcpp::Parameter> & params)
{
  ...
  static const std::unordered_map<std::string, ParamOps> param_ops = {
    ...
    {"publish_low_visibility_voxels",
     {nullptr,
      [this](const rclcpp::Parameter & p) { update_publish_low_visibility_voxels(p); }}},
    {"filter_ratio_error_threshold",
     {validate_normalized,
